...
# LMFL
# https://github.com/Brilhador/tssd2023/blob/main/utils/losses.py
class LargeMarginFocalLoss(nn.Module):
    def __init__(self, lam=0.3, reduction="mean", ignore_index=255):
        ...
        self.lam = lam
        self.focal_crit = smp.losses.FocalLoss(
            gamma=2, mode="multiclass", ignore_index=255
        )
...
class LargeMarginInSoftmaxLoss(nn.Module):
    def __init__(self, lam=0.3, reduction="mean", ignore_index=255):
...
class LargeMarginSoftmaxFuncV2(torch.autograd.Function):
    ...
    def forward(ctx, logits, labels, lam=0.3, focal_flag=False):
        ...
        losses = losses.sum(dim=1).add_(
            nn.functional.cross_entropy(
                logits, labels, reduction="none", ignore_index=255
            )
        )
